...
8. **自动提交**：在 wiki 根目录执行 `git add -A && git commit -m "Query: <问题摘要> → synthesis/<slug>" && git push`
...
7. **更新交叉引用**：确保双向链接
8. **更新 index.md**：运行 `python3 scripts/gen_index.py wiki/` 或手动更新
9. **追加 log.md**：记录本次摄入
10. **校验**：运行 `python3 scripts/okf_check.py wiki/` 验证合规
11. **自动提交**：在 wiki 根目录执行 `git add -A && git commit -m "Ingest: <源标题>" && git push`
...
2. 逐个按上述流程摄入
3. 摄入后统一更新 index.md 和 log.md
4. 运行 `okf_check.py` 和 `link_check.py` 验证
5. **自动提交**：`git add -A && git commit -m "Batch ingest: <N> sources" && git push`
...
5. 用户确认后执行修复
6. 追加 log.md
7. **自动提交**：`git add -A && git commit -m "Lint: <修复摘要>" && git push`
