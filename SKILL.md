...
2. **当前工作目录**：从 CWD 向上找，发现 `.okf-config.json` 或 `wiki/` 即为 wiki 根
3. **常见位置探测**：检查 `~/yjzhuang-wiki`、`~/Documents/my-wiki`、`~/my-wiki` 等常见路径
4. **询问用户**：以上都找不到时，问用户"你的 wiki 在哪个目录？"

确定 wiki 根后，**再确定 bundle 目录名**：
- 读 `<wiki根>/.okf-config.json`，若存在则取其中 `bundle` 字段（如 `"bundle": "yjzhuang"`）
- 否则默认 `wiki/`

不同仓库的 bundle 名可能不同 —— 用户可能起了 `yjzhuang/`、`home/` 之类的名字以便在 Obsidian 多 vault 间区分。后续所有路径以解析结果为准，**不要硬编码 `wiki/`**。

路径约定：
- **Wiki 根**：包含 `raw/`、`<bundle>/`、`scripts/`、`AGENTS.md` 的目录
- **Bundle 根**：`<wiki根>/<bundle>/`，OKF 知识页面所在。默认 `wiki/`，可配置
- **原始资源**：`<wiki根>/raw/`（只读，不可修改）
- **操作手册**：`<wiki根>/AGENTS.md`（完整工作流和页面模板，操作前先读）

确定后，记住 wiki 根和 bundle 名供本次会话后续使用。

> 💡 在 wiki 根或其子目录里运行 `scripts/*.py` 时**可以不传路径**，脚本会自己解析。例如 `python3 scripts/okf_check.py` 等价于 `python3 scripts/okf_check.py <bundle>/`。
...
7. **更新交叉引用**：确保双向链接
8. **更新 index.md**：优先运行 `python3 scripts/gen_index.py` 自动生成（推荐，避免遗漏和排序错误）。若脚本不可用需手动更新，应**整体重写 index.md** 而非逐条插入——逐条 `replace_in_file` 在多条目新增时效率低且易出错
9. **追加 log.md**：记录本次摄入
10. **校验**：运行 `python3 scripts/okf_check.py` 验证合规
...
当用户说"批量摄入"时：
1. 运行 `python3 scripts/scan_sources.py` 找未处理源
2. 逐个按上述流程摄入
...
1. 运行 `python3 scripts/okf_check.py` — OKF 合规检查
2. 运行 `python3 scripts/link_check.py` — 断链 + 孤儿页
3. AI 审查：
   - 页面间矛盾声明
   - 过时信息（新源已超越旧结论）
   - 重要概念缺独立页面
...
- 这些路径相对于 bundle 根（默认 `wiki/`，也可能是 `yjzhuang/` 等自定义名），**不要把 bundle 目录名写进链接**
- 不要使用 `[[wikilink]]` 语法
- 不要使用 `/wiki/entities/xxx.md`（多了 bundle 名前缀）—— 哪怕 bundle 真叫 `wiki/`，链接里也不能有它
...
scheduled_task_create({
  ...
  schedule: "每天12点",
  workDir: "<wiki根>",
  prompt: "在 <wiki根> 目录下运行以下步骤：\n1. 读 .okf-config.json 确定 bundle 名（默认 wiki/）\n2. 读取 .automation.json 配置文件，检查 auto_ingest 开关状态\n3. 运行 `python3 scripts/scan_sources.py --json`（路径自动解析）\n4. 如果没有未处理的源：通知\"所有源已摄入\"，结束\n5. 如果有未处理的源：\n   - auto_ingest 为 false：推送待摄入列表\n   - auto_ingest 为 true：逐个读取源文件，按 AGENTS.md 工作流生成页面，更新 index.md 和 log.md，校验，git commit & push\n6. 运行 okf_check.py 和 link_check.py 健康检查",
  notifyWecom: true,
...
用户可通过以下命令控制模式（在 wiki 根目录内运行可省略路径）：
- `python3 scripts/auto_toggle.py ingest on` — 自动摄入
- `python3 scripts/auto_toggle.py ingest off` — 仅通知（默认）
- `python3 scripts/auto_toggle.py status` — 查看状态
